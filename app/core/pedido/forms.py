...
class MovimientoForm(ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        print_info('MOVIMIENTO FORM')
        print(usuario)
        super(MovimientoForm, self).__init__(*args, **kwargs)
        if usuario:
            self.fields['solicitante'].queryset = Dependencia.objects.filter(
                sucursal=usuario.sucursal, dependencia_padre__isnull=True, activo__exact=True).order_by('denominacion')
            self.fields['area_solicitante'].queryset = Dependencia.objects.filter(
                sucursal=usuario.sucursal, dependencia_padre__isnull=False, activo__exact=True)
...
class SearchForm(forms.Form):
    ...
    situacion.widget.attrs.update(
        {'class': 'form-control select2', 'multiple': 'true'})
    sucursal.widget.attrs.update(
        {'class': 'form-control select2', 'multiple': 'true'})
    solicitante.widget.attrs.update(
        {'class': 'form-control select2', 'multiple': 'true'})
    area_solicitante.widget.attrs.update(
        {'class': 'form-control select2', 'multiple': 'true'})
